...
% continuum.tex v4.2 (c) | Copyright 2023 Daniel E. Janusch
...
\begin{section}{Introduction and Countability Theorems}\label{sec:background}
	...
	Equations (2a) and (2b) describe what will be called ``sum ring operators," ``product
	...
	\begin{align}
		...
		\\
		R(\vec v\,) & :=\left\{\sum_{n=1}^{\text{len}\,\vec v}(a_n\cdot\vec v_n):a_n\in R\right\}~(2\text a)
		\hspace{2em}
		R[\,\vec v\,]:=\left\{\prod_{n=1}^{\text{len}\,\vec v}(a_n+\vec v_n):a_n\in R\right\}~(2\text b)\\
		\aleph_0 & := |\mathbb N|\\
		C(S) & :=\begin{cases}
	...
	\end{thm}\begin{proof}
		$R^n$ can be factored as $R\times R^{n-1}$ which is countable if $R^{n-1}$ is countable (Theorem~\ref{thm:multiplications}.1). This can be applied recursively until it becomes countable if $R^2$ or $R\times R$ is
