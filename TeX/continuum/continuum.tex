...
\begin{section}{Background and Countability Theorems}\label{sec:background}
	...
	\end{thm}\begin{proof}
		Put ordered pairs using elements all the of $R$ and $S$ into a table as shown
		below and go along the diagonals getting every element in the product and not
		``missing" any. The numbers and colons are the output indices. The first 25
		are labeled if in view. Similar logic works for $R\cdot S$. This is also
		algorithmically viable as shown in Section~\ref{sec:algorithm code}. The output
		indices relate strongly to triangle numbers.
	\end{proof}
