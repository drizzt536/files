...
% process.tex v1.0 (c) | Copyright 2023 Daniel E. Janusch
	...
	\subsection*{Example:}
		...
		\begin{minipage}{0.5\textwidth}
			\begin{tikzpicture}
				\begin{axis}[
						xlabel=$x$,
						ylabel=$\displaystyle\int_{g^{-1}(0)}^x f(t)\text dt$,
						axis lines=middle,
						samples=2000,
						grid=both,
						line width=1.01pt,
					...
					\addplot[color=red, domain=0:10]{(floor(x)^2 - floor(x)) / 2 - (x - floor(x)) * floor(x)};
