...
static Lfloat R(Point p) {
	return (Lfloat) p.x +
		(Lfloat) reverseBase10(p.y) /
		(Lfloat) ullpow10( len(p.y) );
}

static Lfloat getReal(uLong index) {
	bool negative = 0;

	index % 2ull && (negative = 1, index--);
	Point t = generateIndices(index /= 2);

	return (Lfloat) ((negative ? -1 : 1) * R(t.x, t.y));
}

Lfloat *generateRealsSet(uLong maxIndex) {
	Lfloat *set = (Lfloat *) malloc(maxIndex * sizeof(Lfloat));
