...
public:
	...
	FORCE_INLINE void update(u8 r0, u8 r1, u8 r2, u8 r3, u8 r4, u8 r5, u8 r6, u8 r7) {
	...
	FORCE_INLINE void toggleCol(u8 col, u8 mask = 0xFF) {
		// mask bit i corresponds to row i
		if (col > 7)
			return;
	...
	FORCE_INLINE void init(u8 DIN, u8 CS, u8 CLK) {
	...
	FORCE_INLINE void init(void) {
		init(MAX7219_DIN, MAX7219_CS, MAX7219_CLK);
	}

	FORCE_INLINE Matrix(u8 DIN, u8 CS, u8 CLK) {
		init(DIN, CS, CLK);
	}

	FORCE_INLINE Matrix(void) {
		init();
	}
} // Matrix
