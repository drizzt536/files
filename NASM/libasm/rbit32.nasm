...
%ifidni RBIT32_OPTIMIZATION, balanced
	...
	; uint32_t rbit32(uint32_t x) {
	;     return rbit16(x) << 16 | rbit16(x >> 16);
	; }
	rbit32: ; ecx is the input.
		mov 	eax, ecx ; save the input value for later
		call 	rbit16   ; reverse the lower 16 bits

		shr 	edx, 16  ; move the upper half of the input
		mov 	cx, dx   ; into the lower half of ecx

		mov 	dx, ax   ; save the output of the previous call (reversed lower half)
		shl 	edx, 16  ; into the upper half of dx

		call	rbit16  ; mov ax, reversed upper half of input
...
%else ;; looped version
	...
	rbit32:
		mov 	r8 , rcx ; save the input value for later. (zero extend to 64 bits)
		xor 	eax, eax ; clear all the bits, so `al`  will be the only stuff used 
		xor 	r9d, r9d ; clear all the bits, so `r9b` will be the only stuff used 

		mov 	rcx, 3   ; one less than the number of iterations required.
	.loop:
		...
		or  	eax, edx ; use rax as dst this time because it is the return register
		ret
