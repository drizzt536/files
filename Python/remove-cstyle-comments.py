...
def remove_comments_file(filepath: str | int, edit: bool = False, /) -> str:
	"""
		takes the file location of a file. removes all C-style comments from
		the file. if the second argument is truthy, The file is overwrited to
		the new state. returns the output string.
	"""
	from automate.misc import read, write

	output = remove_comments_str( read(filepath) )
	if edit:
		write(filepath, output, "w")
