...
/* Customization & Constants: */ {
	...
	LibSettings.Logic_Comparatives_Argument       === "default" && (LibSettings.Logic_Comparatives_Argument       = !1);
	...
	/* Local Variables (may also be global) */ {
		...
		var stringifyScope = function stringifyScope(val, scopename, list, origStr) {
			...
			parse: {
				...
				value: function stringify(
					...
					switch (object == null ?
						...
						case "symbol": return symbolToString(object);
						...
						case "object":
							...
							for (var output = `{${spacesAtEnds ? space : ""}`, i = 0 ;;) {
								...
								output += `${
									...
									typeof keys[i] === "symbol" ? symbolToString(keys[i], 1) : keys[i]
								}${
									typeof keys[i] === "symbol" ? "]" : '"'
								}:${
									space
		...
		, sizeof = function	sizeof(obj, keys=true) {
			...
			function isInvalidString(str, nextCharacter="]") {
				...
				// uses eval, but can only ever evaluate objects, not statements.
			...
			return typeof n === "number" ?
				...
				typeof n === "bigint" ?
					n :
					typeof n === "string" ?
						// TODO: fix for 0.49999999999999999
						round(+n) :
						NaN;
		}, randint = function randomInt(min=1, max=null) {
			// always returns an integer. can return negative values.
			// randint()     returns from [0, 1]
			// randint(a)    returns from [0, a]
			// randint(a, b) returns from [a, b]
			if (max == null)
				max = min, min = 0;

			if (typeof min !== "number" || typeof max !== "number")
				return round( rand() ); // x \[Element] {0, 1}
			min < 0 && min--;
		...
		}, qsort = (function quickSort_closure() {
		...
		})(), msort = (function mergeSort_closure() {
		...
		var createElement = function createElement(element, options={}) {
			...
			if (typeof element !== "string")
				throw Error`element name is not a string`;
			...
			for (let [name, value] of Object.entries(options)) {
				if (/^on[a-z]+$/.test(name)) {
					...
					if (options.on == null)
						options.on = { [type]: [value] };
					else if (options.on[type] == null)
						options.on[type] = value;
					else if (options.on[type] instanceof Array)
						options.on[type] = options.on[type].concat(value);
					else
						throw Error`options.on[type] must be an array`;
				...
				switch (name) {
				case "on":
					// do listeners last because of options like
						// "onload" or "onclick"
					break;
				case "children":
			...
			if (options.on != null)
			for (const [type, arr] of Object.entries(options.on))
			for (const { type: undefined, listener, handler,
				...eventOptions } of isArr(arr) ? arr : [arr])
				element.addEventListener(type, listener ?? handler, eventOptions);

			for (const object of objects)
				object.appendChild(element);

			element.click(clickTimes);
	...
	, valueCompare(
		...
		if (Number.isNaN(b)) return false; // a is not NaN from the last check
		if (a === undefined) return b === undefined;
		if (a === null) return b === null;
		if (b == null) return false; // a is already not `null` or `undefined`

		if (a.constructor !== b.constructor) return false;


		if ([Boolean, BigInt, String].includes(a.constructor))
			return a === b;
	...
	, "object Object": {
		...
		, "property printStr": (function printString_closure() {
			...
			return function printString(/*mapper, joiner*/) {
				/** valid arguments:
				 *		someArray.printString()
				 *		someArray.printString(joinerString)
				 *		someArray.printString(mapper, joinerString)
				*/

				// no arguments
				if (arguments[0] == null)
					var [mapper, joiner] = [defaultMapper, ","];

				// 1 argument
				else if (arguments[1] == null)
					var [mapper, joiner] = [defaultMapper, arguments[0]];

				// 2 arguments
				else
					var [mapper, joiner] = [arguments[0], arguments[1]];

				// this.map(mapper).join(joiner)
				console.log(
					Array
						.prototype
						.map
		...
		, "overwrite property sort": (function sort_closure() {
		...
		, "property shuffle": function shuffle(times=1) {
			...
			while (times --> 0) {
				for (i = out.length, tmp = []; i --> 0 ;)
					tmp.push( out.removeAt(randint(i)) );
		...
		constructor(help = LibSettings.bMath_Help_Argument) {
			help === "default" && (help = !0);
		...
		piApprox = (function piApprox_closure() {
		...
		_primeFactor = (function primeFactor_closure() {
		...
		constructor(help = LibSettings.cMath_Help_Argument) {
			this.Complex = class Complex {
				...
				toArr() {
					...
					// return Object.values( this.toRegObj() )
				...
				conjugate(createNew=true) {
					if (createNew)
						return new this.constructor(this.re, -this.im);

					this.im *= -1;
				}
				fpart(createNew=true) { return this.frac(createNew) }
			...
			this.eq = {
				...
				, eq(...nums) {
					return nums.map(e => {
						e = typeof e === "number" ?
							this._parent.new(e, 0) :
							e;

						if (type(e, 1) !== "complex")
							throw Error`all arguments must either be complex or real`;
				...
				, close(a, b, range/*=Number.EPSILON*/)  {
					if (typeof a === "number") a = this._parent.new(a, 0);
					if (typeof b === "number") b = this._parent.new(b, 0);

					if (type(a, 1) !== "complex" ||
						type(b, 1) !== "complex")
						throw Error`both arguments must either be complex or real`;

					return  this._parent.rMath.isClose(a.re, b.re, range) &&
							this._parent.rMath.isClose(a.im, b.im, range);
				}
				, ne(a, b)  {
					if (typeof a === "number") a = this._parent.new(a, 0);
					if (typeof b === "number") b = this._parent.new(b, 0);
		...
		cis(/*z*/) {
			// [cos + i sin](x)
			return this.expi.apply(this, arguments);
		}
		abs(z) {
		...
		sgn(z) {
			typeof z === "number" && (z = this.new(z, 0));
			if (type(z, 1) !== "complex") return NaN;
			return this.eq.eq(z, 0) ?
				z :
				this.div(z, this.abs(z) );
			// exp(i arg num);
			// |z| sgn(z) = z <==> sgn(z) = z/|z|
		}
		sign(/*z*/) { return this.sgn.apply(this, arguments) }
		nthrt(z, rt=2, n=0) {
			typeof z === "number" && (z = this.new(z, 0));
			...
			if (type(rt, 1) === "complex") {
				...
				return this.pow(z, this.new(rt.re, -rt.im).div(c2d2));
			}
			if (typeof rt !== "number") return NaN;

			// r e^(iθ) = e^(iθ / root) r^(1 / root)
			return this.expi(this.arg(z, n) / rt).mul(
				rMath.nthrt(z.abs(), rt)
			);
		}
		sqrt(z) {
		...
		constructor(help = LibSettings.fMath_Help_Argument) {
			help === "default" && (help = !0);
	...
	if (LibSettings.Use_Document) {
		...
		function removeEventListener(type, listener=null, options={
			...
			// TODO: make sure this is correct.
				// I think it might have to check .object against this.
			...
			if (valueCompare(listeners[type][i], options))
		...
		EventTarget.prototype.ael =
			EventTarget.prototype.addEventListener =
			addEventListener;
		EventTarget.prototype.rel =
			EventTarget.prototype.removeEventListener =
			removeEventListener;
		EventTarget.prototype.gel =
			EventTarget.prototype.getEventListeners =
			getEventListeners;
		EventTarget.prototype.gml =
			EventTarget.prototype.getMyEventListeners =
			getMyEventListeners;

		HTMLElement.prototype.click = click;
		Document.prototype.click = function click(times=1) {
			return globalThis.document.head.click(times);
		}
		// document.all == null but !== undefined ??!? for some reason.
		document.doctype && document.all !== void 0 && (
			document.all.doctype = document.doctype);

		if (LibSettings.Creepily_Watch_Every_Action &&
			LibSettings.Creepily_Watch_Every_Action !== "default"
		) {
			const tempFunction = evt => {
				console.log(evt.type);
				globalThis[evt.type + "_evt"] = evt;
			};
			for (const str of [
				"click", "dblclick", "auxclick", "contextmenu", "mousemove",
				"drop", "mousedown", "mouseup", "mouseover", "mouseout", "keyup",
				"mouseenter", "mouseleave", "wheel", "pointerover", "pointerout",
				"pointerenter", "pointerleave", "pointerdown", "pointerup",
				"pointermove", "drag", "pointercancel", "gotpointercapture",
				"dragstart", "dragend", "copy", "dragover", "dragenter", "paste",
				"dragleave", "keypress", "keydown", "beforecopy", "beforepaste",
				"beforecut", "input", "devicemotion", "deviceorientation",
				"DOMContentLoaded", "scroll", "touchstart", "touchmove",
				"touchend", "touchcancel"
			])
				document.ael(str, tempFunction);

			for (const str of ["load", "focus", "resize", "blur"])
				globalThis.ael(str, tempFunction);
		}
