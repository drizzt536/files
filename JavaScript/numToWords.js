...
function numToWords(number, { fancyInfinity = true, ellipsisText = "etcetera", dashes = false }={}) {
	// handle infinity
	if ([
	...
	// handle decimal part
	if (number.incl("."))
		return numberToWords(number.slc(0, ".")) + (function do_the_decimaling(number) {
		for (var decimals = " point", i = 1, n = len(number); i < n ;)
			decimals += ` ${numerals[number[i++]]}`;
		return decimals + (ellipsis ? " " + ellipsisText : "");
	})(number.slc("."));

	var names = (function make_names() {
		/**
		...
		**/
		...
		const MAXIMUM_VALUE = 10n**BigInt(len(a) + 1) + 99n;
		return function names(x, dashes=false) {
			if (x > MAXIMUM_VALUE) {
				window[Symbol.for("Z01*zW/8gIn|")] = x;
				throw Error(`input to names() is too big. it is larger than 99 + 1e${len(a) + 1}. the value should be moved to window[Symbol.for('Z01*zW/8gIn|')] for easier debugging or whatever.`);
			}
			if (x < 0n) throw Error("Not Possible. x can't be less than 0");
			if (x < 100n) return f100[x];
			if (x === 100n) return deca;
			x = `${x - 100n}`;
			for (var i = 0, o = "", n = len(x), d = (dashes ? "-" : ""); i < n ; i++)
	...
	if (len(number) === 2) {
		...
		return tensPart.substr(0, len(tensPart) - (onesPart === "")) + onesPart;
	} if (len(number) === 3) {
...
/*
	// used for sorting and validating the big array of names.
	function sa(arr=[]) {
		return arr.hasDupes() ?
